 ...
 *
 * It is text on purpose, even for an app whose users have 64-bit keys:
 * the key type is the app's choice (`keyType: "uuid"` is equally
 * supported), and a `bigInteger` column would make Postgres reject a
 * UUID outright rather than simply not match.
 */
